...
class SmallDilationLastTwoModel(nn.Module):
    """
    A CNN model for chord extraction using dilated convolutions only in the last two layers.
    The first layer uses standard convolution.
    """
    def __init__(self, num_classes, input_height=9, input_width=24):
        super(SmallDilationLastTwoModel, self).__init__()
        ...
        # Layer 1: Standard convolution (no dilation)
        kernel_size1 = 3
        padding1 = 1  # Standard padding for 3x3 kernel
        self.conv1 = nn.Conv2d(in_channels=1, out_channels=12,
                               kernel_size=kernel_size1,
                               padding=padding1)
        self.dropout1 = nn.Dropout2d(p=0.25)

        # Layer 2: Standard kernel size, dilation rate 2
        kernel_size2 = 3
        dilation2 = 2
    ...
    def forward(self, x):
        ...
        # 2. Standard Convolutional Layer
        x = self.conv1(x)       # Shape: (batch, 12, height, width)
        x = torch.relu(x)
        x = self.dropout1(x)

        # 3. Dilated Convolutional Layers (last two)
