...
class MultiDilationMidSqueezeSigmoidChordCNN(nn.Module):
    """
    A CNN model for chord extraction using a multi-dilation block with mid squeeze
    and sigmoid activation to capture features at multiple scales simultaneously.
    """
    def __init__(self, num_classes, input_height=9, input_width=24,
        ...
        super(MultiDilationMidSqueezeSigmoidChordCNN, self).__init__()
    ...
    def forward(self, x):
        ...
        # 4. Mid Squeeze with Sigmoid
        # Apply sigmoid along the feature dimension
        x = torch.sigmoid(x)
