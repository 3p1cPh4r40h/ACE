...
class SmallDilationFirstModel(nn.Module):
    """
    A CNN model for chord extraction using dilated convolution only in the first layer.
    This model uses a dilated convolution (rate=2) in the first layer and standard convolutions
    in subsequent layers to increase the receptive field while maintaining spatial resolution.
    """
    def __init__(self, num_classes, input_height=9, input_width=24):
        super(SmallDilationFirstModel, self).__init__()
        ...
        # Layer 1: Dilated convolution (rate=2)
        kernel_size1 = 3
        dilation1 = 2
