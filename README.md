...
7. Add model loading logic in the `load_model` function in `test_models.py`:
   ```python
   elif model_type == 'your_model':
       model = YourModelClass(num_classes=num_classes)
   ```

8. Create a job script in the `jobs/` folder (e.g., `jobs/your_model.sh`):
   ```bash
   #!/bin/bash
   #SBATCH --job-name=your_model
   #SBATCH --output=logs/your_model-%j.out
   #SBATCH --error=logs/your_model-%j.err
   ...
   # Run the model
   python run_models.py --model_type your_model --epochs 1000 --data_type majmin
   ```

9. Add your job script to the `run_all_jobs.sh` file:
   ```bash
   jobs=(
       # ... existing jobs ...
       "jobs/your_model.sh"
...
10. If your model requires pre-training (like semi-supervised models), update the training logic in `run_models.py`:
    ```python
    if args.model_type == 'semi_supervised' or args.model_type == 'your_model':
        # Pre-training phase (sequence ordering)
        # ... pre-training code ...
    ```

11. Your model should accept the following parameters in its constructor:
...
12. The model's forward pass should:
    - Accept input tensors of shape (batch_size, height, width)
    - Return logits of shape (batch_size, num_classes)
    - For semi-supervised models, optionally accept a `task` parameter for different outputs

13. Common utilities available for models:
